...
and prepopulates a new interval tier ("Phone") with labels appropriate for measuring
VOT:
...
For later analysis, only REL is required, and it is require even if the release burst
is absent (an arbitrarily short interval should be used).  The other intervals may or
may not be present.  See Abramson & Whalen (submitted) for details.
  To use "prepopulate.praat", create labels for a speech file with, at a minimum, an
...
adjust the boundaries than to have to type the label names many times.
  ...
  Once the "Phone" tier is complete, the second procedure, "get_vot.praat", can be
